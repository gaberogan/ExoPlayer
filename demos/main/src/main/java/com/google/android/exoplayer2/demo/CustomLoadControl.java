...
public class CustomLoadControl implements LoadControl {
  ...
  public boolean shouldContinueLoading(
    ...
    // BUGFIX: If half of the target buffer is not filled, we need to start loading
    if (allocator.getTotalBytesAllocated() <= targetBufferBytes / 2) {
      isLoading = true;
    }
    ...
    // BUGFIX: Ensure both time buffer and size buffer are reached before we stop loading
    if (bufferedDurationUs >= maxBufferUs && targetBufferSizeReached) {
      isLoading = false;
    }
